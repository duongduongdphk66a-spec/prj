...
class BookReviewQuerySet(models.QuerySet):
    """Custom QuerySet cho BookReview với các method tiện ích được tối ưu"""
    def published(self): return self.filter(is_published=True)
    def verified_reviews(self): return self.filter(is_verified=True)
    def with_text(self): return self.exclude(review_text='')
    def by_rating(self, min_rating=None, max_rating=None):
        qs = self
        if min_rating is not None: qs = qs.filter(rating__gte=min_rating)
        if max_rating is not None: qs = qs.filter(rating__lte=max_rating)
        return qs
    def recent_reviews(self, days=30): return self.filter(created_at__gte=timezone.now() - timedelta(days=days))
    def with_user_book(self): return self.select_related('user', 'book')
    def popular_reviews(self): return self.filter(helpful_votes__gte=5).order_by('-helpful_votes')

class BookReviewManager(models.Manager):
    def get_queryset(self): return BookReviewQuerySet(self.model, using=self._db)
    def published(self): return self.get_queryset().published()
    def verified(self): return self.get_queryset().verified_reviews()
    def get_book_stats(self, book_id):
        """Lấy thống kê đánh giá cho 1 cuốn sách với cache"""
        cache_key = f'book_stats_{book_id}'
        stats = cache.get(cache_key)
        if stats is None:
            stats = self.filter(book_id=book_id, is_published=True).aggregate(
                avg_rating=Avg('rating'), total_reviews=Count('id'),
                five_star=Count('id', filter=Q(rating=5)), four_star=Count('id', filter=Q(rating=4)),
                three_star=Count('id', filter=Q(rating=3)), two_star=Count('id', filter=Q(rating=2)),
    ...
    def get_user_reviews(self, user_id, limit=20):
        ...
        return self.filter(user_id=user_id).select_related('book').order_by('-created_at')[:limit]

class BookReview(SoftDeleteModel):
    """Đánh giá sách từ người dùng với tính năng nâng cao và tối ưu"""
    RATING_CHOICES = [(i/2, f'{i/2} sao') for i in range(1, 11)]   
    MODERATION_STATUS = [('pending', 'Pending Review'), ('approved', 'Approved'), ('rejected', 'Rejected')]
    
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='reviews', db_index=True)
    book = models.ForeignKey('inventory.Book', on_delete=models.CASCADE, related_name='reviews', db_index=True)
    rating = models.DecimalField(max_digits=2, decimal_places=1, choices=RATING_CHOICES, validators=[MinValueValidator(0.5), MaxValueValidator(5.0)], verbose_name="Đánh giá")
    review_text = models.TextField(blank=True, validators=[MinLengthValidator(10)], verbose_name="Nội dung đánh giá")
    title = models.CharField(max_length=200, blank=True, verbose_name="Tiêu đề đánh giá")
    is_spoiler = models.BooleanField(default=False, verbose_name="Có spoiler", db_index=True)
    is_published = models.BooleanField(default=True, verbose_name="Đã xuất bản", db_index=True)
    is_verified = models.BooleanField(default=False, verbose_name="Đã xác thực", db_index=True)
    helpful_votes = models.PositiveIntegerField(default=0, verbose_name="Lượt hữu ích", db_index=True)
    unhelpful_votes = models.PositiveIntegerField(default=0, verbose_name="Lượt không hữu ích")
    reading_progress = models.CharField(max_length=20, choices=[('completed', 'Đã đọc xong'), ('reading', 'Đang đọc'), ('dnf', 'Không đọc nữa')], default='completed', verbose_name="Tiến độ đọc")
    moderation_status = models.CharField(max_length=10, choices=MODERATION_STATUS, default='pending', verbose_name="Trạng thái duyệt", db_index=True)
    moderated_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='moderated_reviews') 
    report_count = models.PositiveIntegerField(default=0, db_index=True)
    helpfulness_score = models.FloatField(default=0.0, db_index=True)  # Denormalized field
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['book', 'is_published', 'rating']),
            models.Index(fields=['user', 'created_at']),
            models.Index(fields=['helpful_votes', 'created_at']),
            models.Index(fields=['moderation_status', 'created_at']),
            models.Index(fields=['helpfulness_score']),
            models.Index(fields=['report_count']),
        ]

    def __str__(self): return f'{self.user.username} - {self.book.title} ({self.rating}⭐)'

    def get_absolute_url(self):
        # Giả định URL chi tiết sách
        return reverse('inventory:book_detail', kwargs={'pk': self.book.pk}) + f'#review-{self.id}'
    ...
    def update_helpfulness_score(self):
        """Cập nhật helpfulness score denormalized"""
        total = self.helpful_votes + self.unhelpful_votes
        if total > 0:
            self.helpfulness_score = (self.helpful_votes / total) * 100
        else:
            self.helpfulness_score = 0.0
        self.save(update_fields=['helpfulness_score'])

    def clean(self):
        if self.review_text and len(self.review_text) < 10:
            raise ValidationError({'review_text': 'Đánh giá phải có ít nhất 10 ký tự'})
        if not self.title and self.review_text:
            self.title = f"Đánh giá về {self.book.title}"
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(*args, **kwargs)
        if is_new:
            from .tasks import update_book_rating
            update_book_rating.delay(self.book.id)

class ReviewHelpfulness(TimestampedModel):
    """Theo dõi việc vote helpful/unhelpful cho review với tối ưu"""
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='review_votes')
    review = models.ForeignKey(BookReview, on_delete=models.CASCADE, related_name='votes')
    ...
    def save(self, *args, **kwargs):
        ...
        old_vote = None
        if not is_new:
            old_vote = ReviewHelpfulness.objects.get(pk=self.pk).is_helpful    
        super().save(*args, **kwargs)
        
        # Sử dụng raw SQL cho performance tốt hơn
        if is_new:
            if self.is_helpful:
                BookReview.objects.filter(id=self.review_id).update(helpful_votes=F('helpful_votes') + 1)
            else:
                BookReview.objects.filter(id=self.review_id).update(unhelpful_votes=F('unhelpful_votes') + 1)
        elif old_vote != self.is_helpful:
            if self.is_helpful:
                BookReview.objects.filter(id=self.review_id).update(helpful_votes=F('helpful_votes') + 1, unhelpful_votes=F('unhelpful_votes') - 1)
            else:
                BookReview.objects.filter(id=self.review_id).update(helpful_votes=F('helpful_votes') - 1, unhelpful_votes=F('unhelpful_votes') + 1)
        
        # Cập nhật helpfulness score async
        self.review.update_helpfulness_score()
...
# Notification model has been merged into UserNotification in notifications app
...
# ... (Các signals khác giữ nguyên) ...
@receiver(post_save, sender=BookReview)
def update_book_stats_cache(sender, instance, created, **kwargs):
    """Xóa cache khi có review mới"""
    if created:
        cache.delete(f'book_stats_{instance.book.id}')



class BatchOperationsMixin:
    @classmethod
    def bulk_update_helpfulness(cls, review_ids):
        if not review_ids:
            return
        from django.db import connection
        # MySQL-compatible: dùng IN thay vì ANY (PostgreSQL)
        placeholders = ','.join(['%s'] * len(review_ids))
        with connection.cursor() as cursor:
            cursor.execute(f"""
                UPDATE blog_bookreview 
                SET helpfulness_score = CASE 
                    WHEN (helpful_votes + unhelpful_votes) > 0 
                    THEN (helpful_votes * 100.0) / (helpful_votes + unhelpful_votes)
                    ELSE 0.0 
                END
                WHERE id IN ({placeholders})
            """, review_ids)

BookReview.batch_ops = BatchOperationsMixin
